...
premium_data <- premium_data %>% 
  mutate(SEX_A = case_when(SEX_A == 1 ~ "M", 
         ...
         EDUCP_A = case_when(EDUCP_A == 0 ~ "Never/KG",
                             ...
                             .default = NA),
         EMPLASTWK_A = case_when(EMPLASTWK_A == 1 ~ "Yes",
                                 EMPLASTWK_A == 2 ~ "No",
                                 .default = NA),
         EMPNOWRK_A = case_when(EMPNOWRK_A == 1 ~ "Yes",
                                EMPNOWRK_A == 2 ~ "No",
                                .default = NA),
         EMPWHYNOT_A = case_when(EMPWHYNOT_A == 1 ~ "Unemployed",
                                 EMPWHYNOT_A == 2 ~ "Contract work",
                                 EMPWHYNOT_A == 3 ~ "Retired",
                                 EMPWHYNOT_A == 4 ~ "Unable for health reasons",
                                 EMPWHYNOT_A == 5 ~ "Taking care of house/family",
                                 EMPWHYNOT_A == 6 ~ "School",
                                 EMPWHYNOT_A == 7 ~ "Working at family business for no pay",
                                 EMPWHYNOT_A == 8 ~ "Other",
                                 .default = NA),
         EMPWHENWRK_A = case_when(EMPWHENWRK_A == 1 ~ "Within last 12 months",
                                  EMPWHENWRK_A == 2 ~ "1-5 years ago",
                                  EMPWHENWRK_A == 3 ~ "Over 5 years ago",
                                  EMPWHENWRK_A == 4 ~ "Never worked",
                                  .default = NA),
         ...
         PREDIB_A = case_when(PREDIB_A == 1 ~ "Yes",
                              PREDIB_A == 2 ~ "No",
                              .default = NA),
         GESDIB_A = case_when(GESDIB_A == 1 ~ "Yes",
                              GESDIB_A == 2 ~ "No",
                              .default = NA),
         ...
         ASEV_A = case_when(ASEV_A == 1 ~ "Yes",
                            ASEV_A == 2 ~ "No",
                            .default = NA),
         LONGCOVD2_A = case_when(LONGCOVD2_A == 1 ~ "Yes",
                                 LONGCOVD2_A == 2 ~ "No",
                                 .default = NA),
         ...
         URBRRL23 = case_when(URBRRL23 == 1 ~ "Large central metro",
                              ...
                              .default = NA),
         MEDICARE_A = case_when(MEDICARE_A == 1 ~ "Yes, info",
                                MEDICARE_A == 2 ~ "Yes no info",
                                MEDICARE_A == 3 ~ "No",
                                .default = NA),
         MCPART_A = case_when(MCPART_A == 1 ~ "Part A - hospital only",
                              MCPART_A == 2 ~ "Part B - medical only",
                              MCPART_A == 3 ~ "Part A & B",
                              .default = NA),
         MCCHOICE_A = case_when(MCCHOICE_A == 1 ~ "Yes",
                                MCCHOICE_A == 2 ~ "No",
                                .default = NA),
         PRIVATE_A = case_when(PRIVATE_A == 1 ~ "Yes, info",
                               PRIVATE_A == 2 ~ "Yes no info",
                               PRIVATE_A == 3 ~ "No",
                                .default = NA),
         EXCHANGE_A = case_when(EXCHANGE_A == 1 ~ "Exchange plan",
                                EXCHANGE_A == 2 ~ "No exchange plan"),
         PLNWRKR1_A = case_when(PLNWRKR1_A == 1 ~ "Employer/union/professional organization",
                                PLNWRKR1_A == 2 ~ "Directly",
                                PLNWRKR1_A == 3 ~ "Obamacare",
                                PLNWRKR1_A == 4 ~ "State/Local/Community program",
                                PLNWRKR1_A == 5 ~ "Other",
                                PLNWRKR1_A == 6 ~ "School",
                                PLNWRKR1_A == 7 ~ "Parents",
                                PLNWRKR1_A == 8 ~ "Relative",
                                .default = NA))

premium_data <- premium_data %>%
  mutate(
    GESDIB_A = if_else(SEX_A == "M", "No", GESDIB_A),
    LONGCOVD2_A = if_else(EVERCOVD_A == "No", "No", LONGCOVD2_A),
    DRK12MWK_A = if_else(DRKLIFE_A == "No", "Did not drink in past year", DRK12MWK_A),
    MCPART_A = if_else(MEDICARE_A == "No", "Not Enrolled", as.character(MCPART_A)),
    MCCHOICE_A = if_else(MEDICARE_A == "No", "Not Enrolled", as.character(MCCHOICE_A)),
    EXCHANGE_A = if_else(PRIVATE_A == "No", "Not Applicable", as.character(EXCHANGE_A)),
    PLNWRKR1_A = if_else(PRIVATE_A == "No", "Not Applicable", as.character(PLNWRKR1_A)),
    EMDOCCUPN2_A = if_else(
      (EMPLASTWK_A == "Yes" | EMPNOWRK_A == "Yes" | EMPWHYNOT_A == "Working at family business for no pay" | EMPWHENWRK_A == "Within last 12 months"),
      as.character(EMDOCCUPN2_A), 
      "99"
    ),
    
    EMDOCCUPN1_A = if_else(
      (EMPLASTWK_A == "Yes" | EMPNOWRK_A == "Yes" | EMPWHYNOT_A == "Working at family business for no pay" | EMPWHENWRK_A == "Within last 12 months"),
      as.character(EMDOCCUPN1_A), 
      "99"
    ),
    
    EMDINDSTN1_A = if_else(
      (EMPLASTWK_A == "Yes" | EMPNOWRK_A == "Yes" | EMPWHYNOT_A == "Working at family business for no pay" | EMPWHENWRK_A == "Within last 12 months"),
      as.character(EMDINDSTN1_A), 
      "99"
    ),
  )
...
```{r}
backward_select_svy <- function(full_model, design, alpha = 0.05) {
  current_model <- full_model
  
  print(paste("Starting Backward Selection with", length(coef(current_model)), "coefficients..."))
  ...
  while(TRUE) {
    ...
    p_values <- coefs[rownames(coefs) != "(Intercept)", "Pr(>|t|)", drop = FALSE]
    max_p <- max(p_values)
    worst_coef_name <- rownames(p_values)[which.max(p_values)]
    
    if(max_p > alpha) {
      

      model_terms <- attr(terms(current_model), "term.labels")
      

      matched_term <- NULL
      for(term in model_terms) {
        if(substr(worst_coef_name, 1, nchar(term)) == term) {
          if(is.null(matched_term) || nchar(term) > nchar(matched_term)) {
            matched_term <- term
          }
        }
      }
      ...
      if(is.null(matched_term)) {
        print(paste("ERROR: Could not map coefficient", worst_coef_name, "to a term. Stopping."))
        break
      }
      
      print(paste("Dropping term:", matched_term, 
                  "| Triggered by coefficient:", worst_coef_name, 
                  "| p-value:", round(max_p, 4)))
      
      new_formula <- update(formula(current_model), paste("~ . -", matched_term))
      
      if(identical(new_formula, formula(current_model))) {
        print("Error: Formula did not update. Stopping loop.")
        break
      }
      
      current_model <- svyglm(new_formula, design = design, family = Gamma(link = "log"))
      
    } else {
      print("----------------------------------------------------")
      print("Selection Complete: All remaining p-values are < alpha.")
      break
    }
  }
...
final_clean_model <- backward_select_svy(final_gamma_model, subset(svy_design, HICOSTR1_A > 0))
```
```{r}
...
```{r}
forward_select_svy <- function(full_model, alpha = 0.05) {
  
  covariates <- attr(terms(full_model), "term.labels")
  selected_covariates <- c()
  remaining_covariates <- covariates
  form <- "HICOSTR1_A ~ 1"  # start with intercept only

  repeat {
    best_p <- Inf
    best_cov <- NULL
    
    for (c in remaining_covariates) {
      formula <- as.formula(paste(form, "+", c))
      check_model <- svyglm(formula, design = svy_design)
      
      # p-value of the last added covariate (second row)
      p_val <- coef(summary(check_model))[, "Pr(>|t|)"][length(coef(check_model))]
    ...
    # Stop if no covariate has p < alpha
    if (is.null(best_cov) || best_p > alpha) {
      break
    }
    
    # Add the best covariate to the formula
    print(paste("Adding: ",best_cov, "p-value", best_p))
    form <- paste(form, "+", best_cov)
    selected_covariates <- c(selected_covariates, best_cov)
    remaining_covariates <- setdiff(remaining_covariates, best_cov)
  }
  
  return(list(formula = as.formula(form), selected = selected_covariates))
}

# Usage
forward_select_svy(final_gamma_model)

```

```{r}
premium_gamma_model1 <- svyglm(HICOSTR1_A ~ AGEP_A + SEX_A + RACEALLP_A + HISPALLP_A + MARITAL_A + NATUSBORN_A + ORIENT_A + 
                                        POVRATTC_A + RATCAT_A + EDUCP_A + ACCSSINT_A +  
                                        DIBEV_A +  PREDIB_A +  GESDIB_A + HYPEV_A +  ASEV_A +  LONGCOVD2_A +  EVERCOVD_A + ANXFREQ_A + DEPFREQ_A +
                                        SMOKELSEV1_A + SMKCIGST_A + DRKSTAT_A + MODFREQW_A +
                                        REGION + URBRRL23 + EXCHANGE_A + PLNWRKR1_A,
                          design = svy_design, 
                          family = Gamma(link = "log"))

summary(premium_gamma_model1)
```

```{r}
gamma_diag <- data.frame(
  Fitted = fitted(premium_gamma_model1),
  Residuals = residuals(premium_gamma_model1, type = "deviance")
)
...
```{r}
final_gamma_model1 <- svyglm(HICOSTR1_A ~ AGEP_A + SEX_A + HISPALLP_A + MARITAL_A + NATUSBORN_A + ORIENT_A + 
                                        POVRATTC_A + RATCAT_A + EDUCP_A + ACCSSINT_A +  
                                        DIBEV_A +  PREDIB_A +  GESDIB_A + HYPEV_A +  ASEV_A +  LONGCOVD2_A +  EVERCOVD_A + ANXFREQ_A + DEPFREQ_A +
                                        SMOKELSEV1_A + SMKCIGST_A + DRKSTAT_A + MODFREQW_A +
                                        REGION + URBRRL23 + EXCHANGE_A + PLNWRKR1_A,
